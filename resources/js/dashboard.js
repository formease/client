...
createProjectBtn.addEventListener('click', async function (e) {
  ...
  if (data && data.projectName.length >= 4 && data.projectName.length <= 40) {
    ...
    const projectElemHTML = `<li data-project="${data.projectName}" role="tab" tabindex="0">${data.projectName}<small>${data.projectDescription}</small></li>`
    ...
    editBtn.addEventListener('click', async () => {
      ...
      if (result.discordWebhook) {
        ...
        projectObj.mainProjectElem.querySelector('.discord-webhook-link').value =
